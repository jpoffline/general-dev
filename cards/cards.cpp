...
class CARD{
  ...
  public:
    ...
    CARD(){};
    CARD(SUIT suit, int value){set_suit(suit); set_value(value);}
    ~CARD(){};
    ...
    void print(){
      std::cout << suit_conv() << " " << value_conv() << std::endl;
    }
    std::string print2(){
      return suit_conv()  + " " + value_conv();
    }
  private:
    SUIT _suit;
    int _value;
    ...
    std::string value_conv(){
      if(_value > 0 && _value < 11) return std::to_string(_value);
      if(_value == 0) return "A";
      if(_value == 11) return "J";
      if(_value == 12) return "Q";
      if(_value == 13) return "K";
      return "NA";
    }
...
class DECK{
  ...
  public:
    ...
    void print(){
      ...
      for(int i = 0; i < _cards.size(); i++)_cards[i].print();
    }
  ...
  public:
    ...
    void play_turn(int tid){
      ...
      for(int h = 1; h < _hands.size(); h++){
        ...
        std::cout << "THIS CARD: " << _hands[h].get_card(tid).print2() << std::endl;
